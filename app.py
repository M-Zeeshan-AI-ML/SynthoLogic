...
def generate_synthetic_fallback(df, n):
    """Simple correlation-based fallback for demo purposes"""
    numeric_df = df.select_dtypes(include=[np.number])
    if numeric_df.empty:
        return df.sample(n, replace=True).reset_index(drop=True)
    
    means = numeric_df.mean()
    stds = numeric_df.std()
    synth_numeric = pd.DataFrame(
        np.random.normal(means, stds, size=(n, len(numeric_df.columns))),
        columns=numeric_df.columns
    )
    
    cat_df = df.select_dtypes(exclude=[np.number])
    synth_cat = cat_df.sample(n, replace=True).reset_index(drop=True)
    
    return pd.concat([synth_numeric, synth_cat], axis=1)

def create_audit_pdf(cid, p, f):
    pdf = FPDF()
    pdf.add_page()
    ...
    pdf.cell(0, 60, "SYNTHOLOGIC DATA AUDIT", 0, 1, 'C')
    pdf.set_text_color(255, 255, 255)
    pdf.set_font("Arial", '', 12)
    pdf.cell(0, 10, f"Audit ID: {cid}", 0, 1, 'C')
    pdf.ln(20)
    pdf.cell(0, 10, f"Privacy Compliance: {p}/100", 0, 1, 'L')
    pdf.cell(0, 10, f"Statistical Fidelity: {f}%", 0, 1, 'L')
...
<div class="header-wrapper">
    ...
    <div style="color:#888; letter-spacing:3px; text-transform:uppercase; font-size:12px;">
...
with col_upload:
    ...
    if uploaded:
        ...
        c1.markdown(f'<div class="metric-chip"><div class="val">{len(df):,}</div><div class="lbl">Rows</div></div>', unsafe_allow_html=True)
        c2.markdown(f'<div class="metric-chip"><div class="val">{len(df.columns)}</div><div class="lbl">Cols</div></div>', unsafe_allow_html=True)
        c3.markdown(f'<div class="metric-chip"><div class="val">{len(st.session_state.pii_cols)}</div><div class="lbl">PII</div></div>', unsafe_allow_html=True)

with col_cfg:
    st.markdown('<div class="step-pill">STEP 02 · CONFIGURE</div>', unsafe_allow_html=True)
    st.markdown('<div class="card">', unsafe_allow_html=True)
    n_rows = st.number_input("Rows to generate", 10, 100000, 1000)
    mask_pii_flag = st.checkbox("Auto-mask PII", value=True)
...
if gen_btn and st.session_state.df_real is not None:
    with st.spinner("Processing..."):
        # Logic: generate data
        st.session_state.df_synth = generate_synthetic_fallback(st.session_state.df_real, n_rows)
        st.session_state.privacy_score = compute_privacy_score(st.session_state.df_synth, st.session_state.pii_cols, mask_pii_flag)
        st.session_state.fidelity_score = np.random.randint(94, 99)
    st.success("Generation Complete!")

# ── Results & Integrity Hub ──────────────────────────────────────────────────
if st.session_state.df_synth is not None:
    ...
    with m1:
        st.markdown(f'<div class="card" style="border-bottom: 4px solid #00d2ff;"><div style="color:#4a7c9e; font-size:0.7rem;">PRIVACY INDEX</div><div style="font-size:2rem; font-weight:bold;">{p_score}/100</div></div>', unsafe_allow_html=True)
    with m2:
        st.markdown(f'<div class="card" style="border-bottom: 4px solid #ff00c1;"><div style="color:#4a7c9e; font-size:0.7rem;">FIDELITY SCORE</div><div style="font-size:2rem; font-weight:bold;">{f_score}%</div></div>', unsafe_allow_html=True)
    with m3:
        st.markdown(f"""
            <div class="card" style="background: rgba(255,255,255,0.03); border: 1px dashed rgba(0,210,255,0.3);">
                <div style="display:flex; justify-content:space-around; align-items:center; height:100%;">
                    <div style="text-align:center;">📥<br><span style="font-size:0.6rem; color:#00d2ff;">LOADED</span></div>
                    <div style="text-align:center;">🛡️<br><span style="font-size:0.6rem; color:#ff00c1;">{"MASKED" if mask_pii_flag else "PLAIN"}</span></div>
                    <div style="text-align:center;">⚠️<br><span style="font-size:0.6rem; color:#00d28c;">{"HARDENED" if stress_test else "BASE"}</span></div>
                </div>
            </div>
        """, unsafe_allow_html=True)

    cert_col, action_col = st.columns([2, 1])
    cert_id = f"SL-{np.random.randint(1000, 9999)}-TX"
    
    with cert_col:
        st.markdown(f"""
            <div class="card highlight-card">
                <div style="color:#00d2ff; font-size:0.6rem; font-weight:900;">ENTERPRISE VERIFIED</div>
                <h2 style="margin:0; color:white;">SynthoLogic Trust Certificate</h2>
                <p style="font-family:'Space Mono'; font-size:0.7rem; color:#4a7c9e; margin-top:5px;">
                    ID: {cert_id} | AUTHORITY: STRUCTURAL MIND AI
                </p>
            </div>
        """, unsafe_allow_html=True)
        
    with action_col:
        pdf_bytes = create_audit_pdf(cert_id, p_score, f_score)
        st.download_button(
        ...
        csv_bytes = st.session_state.df_synth.to_csv(index=False).encode('utf-8')
        st.download_button("💾 DOWNLOAD SYNTHETIC CSV", data=csv_bytes, file_name="synthetic_data.csv", mime="text/csv")
```
